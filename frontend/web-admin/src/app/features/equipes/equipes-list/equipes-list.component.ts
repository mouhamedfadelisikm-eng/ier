...
import { Equipe, EquipePage } from '../../../core/models/equipe.model';
...
export class EquipesListComponent implements OnInit{
  ...
  meta=signal<EquipePage['meta']>(undefined);
  search=signal('');
  ...
  load(page:number){this.isLoading.set(true);this.errorMessage.set(null);this.service.getAll(page).subscribe({next:r=>{this.equipes.set(r.data||[]);this.meta.set(r.meta);this.isLoading.set(false);},error:()=>{this.isLoading.set(false);this.errorMessage.set('Impossible de charger les équipes.');}});}
